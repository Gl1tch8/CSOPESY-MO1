...
struct Config {
    ...
    uint64_t maxOverallMem    = 16384; // bytes; total physical memory size (numFrames = maxOverallMem / memPerFrame)
    uint32_t memPerFrame      = 16;    // bytes; frame size for MemoryManager's paging allocator
